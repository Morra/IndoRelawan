...
function convert_input(input)
{
	...
	var result = input.substr(0,1).toUpperCase() + input.substr(1);
	return result;
}
...
function validation_check(field){
	...
	switch(field){
		...
		case 'checkbox':
		case 'dropdown':
			allowed_validations = ['not_empty', 'nothing'];
			break;
		case 'password':
			allowed_validations = ['is_numeric', 'not_empty', 'min_length', 'max_length'];
			break;
		case '':
			break;
		default:
			allowed_validations = ['is_numeric', 'not_empty', 'is_email', 'min_length', 'max_length'];
			break;
	}
...
(function($){
	...
	{
		$('form#TypeFormPopupForm').live("keypress",function(){
			$('div#form-error').hide();
		});
	    $('form#TypeFormPopupForm').live("submit",function(e){
			e.preventDefault();	
			...
			$.post(action,myData,function(data){
				...
				{	
					$('div#form-error').html('Missing required fields.');
					$('div#form-error').show();
					$.colorbox.resize();
				}
				...
				else
				{
					if(this_element == '')
					{
						...
						content += '<td>'+convert_input(data.input_type)+'</td>';
						content += '<td>';
		...
		$('input[type=text].input_attribute').live("keyup" , function(){
			if($.trim($(this).val()) == "")
			{			
		...
		$('input[type=checkbox].input_attribute').live("change" , function(){
			if(!$(this).is(':checked'))
			{				
				//$(this).next().next().val('');
...
//		$('table a.popup.url.edit-field').live('click' , function(e){
		$(document).delegate('table a.popup.url.edit-field','click' , function(e){
			e.preventDefault();
			...
			// $(document).bind('cbox_complete', function(){
				// var INPUTS = $('#cboxLoadedContent #TypeFormPopupForm [name^=data]');
				// INPUTS.each(function(i, el){
					// $(el).val(ROW.find('input[type=hidden]').val());
				// });
			// });
		});
	});
