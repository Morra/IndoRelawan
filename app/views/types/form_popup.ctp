...
<div id="upload-popup" class="upload-popup form-popup">
	<?php
		...
		echo $form->create('Type', array('id'=>'TypeFormPopupForm','action'=>'form_popup/'.$state,'type'=>'file','class'=>'notif-change form-horizontal fl','inputDefaults' => array('label' =>false , 'div' => false)));
	?>
			...
			<div class="popup-content">
				...
				<fieldset>
					<div class="control-group" <?php echo ($state=='add'?'':'style="display:none"'); ?>>            
						<label class="control-label">Field Label*<span></span></label>
						<div class="controls">
							<input id="myLabel" name="data[TypeMeta][key]" type="text" class="large" value="" />
							...
							<select id="input_type" name="data[TypeMeta][input_type]" class="field_type" <?php echo ($state=='add'?'':'DISABLED'); ?>>
								<?php
									foreach ($inputType as $key10 => $value10)
									{
										...
										{
											echo "<option SELECTED value=\"".$value10."\">".convert_input($value10)."</option>";
										}
										else
										{
											echo "<option value=\"".$value10."\">".convert_input($value10)."</option>";
										}
									}
								?>
							</select>
					...
					<div class="control-group options">            
						<div class="controls">
							<textarea id="value" name="data[TypeMeta][value][option]" class="large" value="" rows=2></textarea>
					...
					<div class="validation">
						...
						<div class="control-group nothing" style="margin-top: 10px;">
							<div class="controls inline" style="font-style: italic;">No validation options available</div>
						</div>
					</div>
				</fieldset>
			</div>
